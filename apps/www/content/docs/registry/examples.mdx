...
### Custom style that extends miraicn/ui

The following registry item is a custom style that extends miraicn/ui. On `npx shadcn init`, it will:
...
The following registry item is a custom style that doesn't extend miraicn/ui. See the `extends: none` field.
...
- Add the `utils` registry item from the miraicn/ui registry.
...
The following style will init using miraicn/ui defaults and then add a custom `brand` color.
...
This blocks installs the `login-01` block from the miraicn/ui registry.
...
You can install a block from the miraicn/ui registry and override the primitives using your custom ones.

On `npx shadcn add`, the following will:

- Add the `login-01` block from the miraicn/ui registry.
