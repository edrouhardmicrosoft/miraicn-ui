...
Used for registry dependencies. Can be names or URLs. Use the name of the item to reference miraicn/ui components and urls to reference other registries.

- For `miraicn/ui` registry items such as `button`, `input`, `select`, etc use the name eg. `['button', 'input', 'select']`.
